import { X } from 'lucide-react';
import type { ReactNode } from 'react';

interface FilterChipProps {
  label: string;
  value: ReactNode;
  onClear: () => void;
}

/** Shared "Filtered by <label> <value>" pill shell with a clear button —
 *  used by `ActiveFilterChip` (checkbox filters) and `DateRangeFilterChip`
 *  (date-range filters), which only differ in how they compute `value`. */
export function FilterChip({ label, value, onClear }: FilterChipProps) {
  return (
    <div className="flex items-center gap-1 rounded-full border border-[#e5e7eb] bg-[#f9fafb] px-3 py-1.5 text-[13px]">
      <span className="text-[#9ca3af]">{label}</span>
      <span className="font-semibold text-[#0a0a0a]">{value}</span>
      <button
        type="button"
        aria-label={`Clear ${label.toLowerCase()} filter`}
        onClick={onClear}
        className="ml-0.5 flex h-4 w-4 items-center justify-center rounded-full text-[#9ca3af] hover:bg-[#e5e7eb] hover:text-[#0a0a0a]"
      >
        <X className="h-3 w-3" strokeWidth={2.5} />
      </button>
    </div>
  );
}
