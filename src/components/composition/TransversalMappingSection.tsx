...
import { TransversalMappingRow, type TransversalMapping } from '@/components/composition/TransversalMappingRow';
            ...
            {mappings.map((m) => {
              ...
              return (
                <TransversalMappingRow
                  key={m.id}
                  mapping={m}
                  layupLabel={layupOptions.find((l) => String(l.id) === m.layupId)?.name}
                  ...
                  onPickLayup={() => setLayupPickerFor(m.id)}
                  editingStart={editingThisStart}
                  editingEnd={editingThisEnd}
                  onStartProfileChange={(v) => {
                    updateMapping(m.id, { startProfileId: v });
                    setEditingProfileFor({ mappingId: m.id, side: 'start' });
                  }}
                  onEndProfileChange={(v) => {
