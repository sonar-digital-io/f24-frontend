...
export function GeometryEdit() {
  ...
  // viewer show a "refreshing" overlay on top of the still-visible stale mesh, distinct
  // from OccViewer's own first-load spinner (which only fires when stlData changes).
  const [resultRegenerating, setResultRegenerating] = useState(false);
  ...
  // Signature (name|date|description) of the last create/update attempt that failed —
  // blurring re-fires this handler on every field, so without this guard a failed save
  // retries in a loop until the user actually changes something.
  ...
  // Autosave the Project configuration tab on every blur, same as Material's Mechanical/
  // Fatigue tabs — creates the geometry as soon as Name is filled in while new, even
  // before Date/Description are, then PUTs on later blurs once created. Name/date/
  // description are still all mandatory for the geometry to be considered complete (see
  // projectConfigValid below) — they just don't gate the create/update call itself.
  async function handleCreatePanelBlur() {
    if (!newName.trim()) return;
    const signature = `${newName.trim()}|${newDate}|${newDescription.trim()}`;
    if (lastFailedAttemptRef.current === signature) return;
    if (isNew) {
      ...
      try {
        ...
        // createMutation.isError already surfaces the failure in the UI — stay on this
        // tab. Remember this attempt so unrelated blurs don't retry it in a loop.
        lastFailedAttemptRef.current = signature;
      } finally {
    ...
    try {
      ...
      // updateGeneralMutation.isError already surfaces the failure in the UI. Remember
      // this attempt so unrelated blurs don't retry it in a loop.
      lastFailedAttemptRef.current = signature;
    }
  }

  // Name/date/description are all mandatory — unlike Material's General tab, autosave
  // itself doesn't wait for all three (see handleCreatePanelBlur above), but the
  // exit-confirm flow below still treats the geometry as incomplete until they are.
  const projectConfigValid = Boolean(newName.trim() && newDate && newDescription.trim());
  ...
  // Gates the other tabs, and drives the toolbar's Saved/Saving indicator — hidden
  // (undefined) rather than "Not saved" while there's nothing in flight to report.
  ...
  // Drives the toolbar's Saved/Saving indicator while on the Profiles tab.
  const profilesStatus: SaveStatus | undefined = updateProfilesMutation.isPending
    ? 'saving'
    : updateProfilesMutation.isSuccess
      ? 'saved'
      : undefined;
  ...
  async function handleGenerateResult(signature: string) {
    ...
    try {
      ...
      if (latestResultRequestRef.current === signature) setResultRegenerating(false);
    }
  }
