...
export function CompositionNew() {
  ...
  const [transversalSaveState, setTransversalSaveState] = useState({
    pending: false,
    error: false,
  });
  const handleTransversalSaveStatusChange = useCallback(
    (status: { pending: boolean; error: boolean }) => setTransversalSaveState(status),
    [],
  );
  ...
  const anyAutosavePending =
    generalSavePending ||
    layupSaveState.pending ||
    layupMappingSavePending ||
    transversalSaveState.pending;
  ...
  async function saveLayupMappingData() {
    ...
    await updateMappingLongitudinalMutation.mutateAsync({
      upper_side: toEntries(nextUpper),
      lower_side: toEntries(nextLower),
    });
    setSavedMappingsSnapshot(mappingsSnapshotKey(nextUpper, nextLower));
  }
  ...
  async function handleTabChange(tab: CompositionTab) {
    // A pending autosave still has in-flight state (e.g. a not-yet-assigned
    // backend id) that a tab switch must not race past.
    ...
    <div className="flex min-h-screen w-full flex-col">
      ...
      <main className="relative flex-1 overflow-hidden bg-[#f8fafc]">
        ...
        <div className="absolute bottom-4 left-4 right-4 top-[60px]">
          ...
          {/* Always mounted — hidden instead of unmounted so a switch away
              mid-debounce doesn't clear the pending autosave's setTimeout
              and silently drop the edit. */}
          <div className={activeTab === 'layup' ? undefined : 'hidden'}>
            <CompositionLayupTab
              compositionId={compositionId}
              layups={layups}
              onAddLayup={addLayup}
