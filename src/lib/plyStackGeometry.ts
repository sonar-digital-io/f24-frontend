...
export function isBiaxial(materialName: string, materials: Material[]): boolean {
...
/**
 * Clears `group` and rebuilds every ply's box mesh + edges + fiber-orientation
 * "sheets" + the top-ply 0° reference arrow/label. Pure Three.js scene
 * mutation — the caller (PlyStackViz) owns disposal bookkeeping and the
 * camera auto-fit that follows.
 */
export function rebuildPlyStackMeshes(group: THREE.Group, plies: Ply[], materials: Material[]) {
